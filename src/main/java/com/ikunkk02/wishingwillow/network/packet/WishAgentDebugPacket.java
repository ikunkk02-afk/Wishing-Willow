...
public record WishAgentDebugPacket(WishAgentDebugSnapshot snapshot) {
    public static void encode(WishAgentDebugPacket packet, FriendlyByteBuf buffer) {
        ...
        buffer.writeUUID(value.sessionId()); buffer.writeEnum(value.mode());
    ...
    public static WishAgentDebugPacket decode(FriendlyByteBuf buffer) {
        ...
        return new WishAgentDebugPacket(new WishAgentDebugSnapshot(session, mode, iterations, calls, tools,
                buffer.readEnum(WishVerificationState.class), buffer.readEnum(WishFinalizationState.class)));
    }
